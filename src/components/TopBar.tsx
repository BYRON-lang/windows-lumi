...
interface TopBarProps {
  background?: string;
  borderBottom?: string;
  hideBorderLeftPx?: number; // masks the bottom border on the left (e.g., over sidebar width)
  onToggleSidebar?: (collapsed: boolean) => void;
  showLogo?: boolean;
  showSidebarToggle?: boolean;
}

export default function TopBar({ background = '#202020', borderBottom, hideBorderLeftPx, onToggleSidebar, showLogo = true, showSidebarToggle = true }: TopBarProps) {
      ...
      <div style={{ display: 'flex', alignItems: 'center', gap: '8px', WebkitAppRegion: 'no-drag' } as React.CSSProperties}>
        {showLogo && (
          <img
            src={logoImage}
            alt="Lumi Logo"
          style={{ width: '22px', height: '22px', filter: 'brightness(0) invert(60%)', userSelect: 'none', marginLeft: '2px' }}
            draggable="false"
          />
        )}
        {showSidebarToggle && (
...
<mask id="SVGTIzOxkmO" fill="#808080">
      ...
      <div style={{ WebkitAppRegion: 'no-drag', marginLeft: 'auto' } as React.CSSProperties}>
        <WindowControls
      ...
      {/* Mask TopBar bottom border over sidebar width when expanded */}
      {hideBorderLeftPx && hideBorderLeftPx > 0 && (
        <div
          style={{
            position: 'absolute',
            left: 0,
            bottom: 0,
            width: hideBorderLeftPx,
            height: 1,
            backgroundColor: background,
            pointerEvents: 'none',
          }}
