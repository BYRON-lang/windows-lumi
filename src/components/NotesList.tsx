...
const NotesList: React.FC = () => {
  ...
  const [notes] = React.useState<NoteItem[]>([
    { id: '1', title: 'Project Ideas', content: 'Brainstorm session for next quarter...', date: new Date('2024-10-16') },
    { id: '2', title: 'Meeting Notes - Q4 Planning', content: 'Discussed budget allocation and team structure...', date: new Date('2024-10-15') },
    { id: '3', title: 'Research Notes', content: 'Latest findings on user behavior patterns...', date: new Date('2024-10-14') }
  ]);
...
// NoteRow component with delete button on hover
const NoteRow: React.FC<{ note: NoteItem }> = ({ note }) => {
    ...
    >
      ...
      >
        <button
          ...
          onClick={(e) => {
            e.stopPropagation();
            console.log('Delete clicked for note:', currentTitle);
          }}
          style={{
            width: 28,
            height: 28,
      ...
      <div style={{
        ...
        <div style={{
          ...
          <div style={{
            ...
            {note.date.toLocaleDateString('en-US', { month: 'short', day: 'numeric', year: 'numeric' })}
          </div>
        </div>
      </div>
    </div>
